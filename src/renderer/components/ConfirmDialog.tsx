...
export default function ConfirmDialog({ open, title, message, confirmLabel = 'Confirm', danger, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div ref={dialogRef} className="relative bg-dark-surface border border-dark-border rounded-lg shadow-xl w-full max-w-sm mx-4 p-5 animate-modal">
        <h3 className="text-base font-semibold mb-2">{title}</h3>
        <p className="text-sm text-dark-muted mb-5">{message}</p>
